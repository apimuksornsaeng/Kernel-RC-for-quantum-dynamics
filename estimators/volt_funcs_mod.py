...
class Volterra:
    ...
    def __init__(self, ld_coef, tau_coef, reg, washout, selected_index, nfeatures=None, pinv=False):
        ...
        self.selected_index = selected_index    # Store a list of selected index(es) for feature selection
        # print(f"Selected index for training and forecasting is {self.selected_index}")
    ...
    def Train(self, training_input_full, training_teacher):
        ...
        Parameters
        ----------
        training_input_full : array_like
            Full training input for training in Volterra. Must have format (nsamples, ndim)
        ...
        if self.selected_index is not None:
            # print(f"Selecting index {self.selected_index} from training input for training")
            # print(f"Training input is of shape {training_input_full.shape} before selecting index {self.selected_index}")
            training_input = training_input_full[:, self.selected_index]
            # print(f"Training input is of shape {training_input.shape} after selecting index {self.selected_index}")
            # print(f"Training input is of shape {training_input.shape} after selecting index {self.selected_index}")
        else:
            training_input = training_input_full
        ...
        # print("Computing Gram matrix and regression weights")
        ...
        # print(f"Performing regression with nfeatures = {self.nfeatures} and washout = {self.washout}")
    ...
    def PathContinue(self, latest_input_full, nhorizon):   
        ...
        Parameters
        ----------
        latest_input_full : array_like
            Full input array to path continue from
        nhorizon : int
        ...
        if self.selected_index is not None:
            # print(f"Selecting index {self.selected_index} from latest input for path continuing with shape {latest_input_full.shape} for path continuing")
            latest_input = latest_input_full[self.selected_index]
        else:
            latest_input = latest_input_full
        ...
        # Initialise store for the forecast output
        output_full = np.zeros((self.nhorizon, self.ntargets))
        output = np.zeros((self.nhorizon, self.ntargets))
        ...
        # Iterate through the testing horizon
        for t in range(self.nhorizon):
            output = output_full[:, self.selected_index] if self.selected_index is not None else output_full
            # if (t+1) % 1000 == 0 or t == 0:
            #     print(f"At time {t + 1} in path continuing, training input shape is {self.training_input.shape}, output shape is {output.shape}, and latest input shape is {latest_input.shape}")
            ...
            # Compute the forecast using the new Gram input column
            for target in range(self.ntargets):
                output_full[t, target] = np.dot(self.alpha[:, target], Gram_new_input_col[self.ninputs-self.nfeatures:self.ninputs]) + self.alpha0[target]
